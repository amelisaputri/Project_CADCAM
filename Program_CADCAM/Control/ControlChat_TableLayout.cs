...
{
    public partial class ControlChat_TableLayout : UserControl
    {
        public ControlChat_TableLayout()
        {
            InitializeComponent();
        }
        ...
        public void SetMessage(string name, string time, string message, bool isSender)
        {
            ...
            // ✨ Set label width limit (fixed width, flexible height)
            LbText.MaximumSize = new Size(340, 0);
            LbText.MinimumSize = new Size(340, 50);// Width fixed at 340, height will grow
            LbText.AutoSize = true;

            // ✨ Let the container panel auto-resize based on its content
            Panel_Bubble.AutoSize = true;
            Panel_Bubble.AutoSizeMode = AutoSizeMode.GrowAndShrink;

            // ✨ Same for the outer control
            this.AutoSize = true;
            this.AutoSizeMode = AutoSizeMode.GrowAndShrink;
            ...
            if (isSender)
            {
                ...
                LbTime.TextAlign = ContentAlignment.BottomLeft;
                LbTime.AutoSize = true;
                LbTime.Location = new Point(0, Panel_Bubble.Height - LbTime.Height);
